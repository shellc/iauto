...
class Session:
    ...
    def react(self, history: int = 5, rewrite: bool = False, log=False, **kwargs):
...
Task: {task}
        ...
        r_pattern = re.compile(r'Retrieve\[(\S+)\]')
        for s in ss:
            s = s.strip()
            if "Observation" not in s:  # drop observation generated by LLM
                steps.append(s)
